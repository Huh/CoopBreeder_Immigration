...
################################################################################
  ...
  #  Model tests for effect of study area (E/W) and harvest (Y/N) on density of
  #  wolves in central Idaho. This includes a random effects for year.
...
################################################################################   
  ...
  model {
      ...
      for(yr in 1:nyear) {
        for(i in 1:nrow) {
          den[yr,i] <- alpha[yr] + B1 * AREA[i] + B2 * HARV[i]
        }
      }
      ...
      #  Specify priors
      #  Fixed effects 
      B1 ~ dnorm(0, 0.35)  # Study area (E/W)
      B2 ~ dnorm(0, 0.35)  # Harvest (Y/N)
      ...
      # for(h in 1:2) {
      #   for(yr in 1:nyear) {
      #     harv.den[h, yr] <- alpha[yr] + B1 * (h - 1) + B2 * (h - 1)
      #   }
      # }
      # #  Mean density for each study area when harvest did and did not occur
      #   for(h in 1:2) {
      #     sa.den[h] <- mean(harv.den[h,])
      #   }
      # }   
          # OH LOOK THIS IS IDENTICAL TO MEAN DENSITY WHEN HARVEST DOES/DOESN'T OCCUR
          # HOW DO I DERIVE THEIR INDIVIDUAL EFFECTS WHILE AVERAGING OVER THE OTHER?
          # WHAT DOES MEAN.DEN[H] ACTUALLY MEAN?!?!?!
      ...
      #  Individual estiamtes for each year and study area
        for(i in 1:nrow) {
          harv.den[i] <- alpha[YR[i]] + B1 * AREA[i] + B2 * HARV[i]
        }
...
################################################################################
  ...
  params <- c("B1", "B2", "alpha", "tau", "sigma", "sd.alpha", "tau.alpha", "harv.den")  #, "mean.den"
  ...
  #  Why is it saying there's a positive effect of harvest and that density 
  #  increased after harvest was initiated? It didn't. 
